...
const AUTO = { off: "不自动总结", abstract: "自动总结·摘要", topN: "自动总结·前 3 条" };
const AUTO_OPTS = [["off", "关闭"], ["abstract", "仅摘要"], ["topN", "Top-3"]];
...
function DigestItem({ p, fetchedMeta, fetchingMeta, onFetch, onReadPaper, onRead, pushToast }) {
  ...
  const doSum = async () => {
    setSumming(true);
    try { const r = await bridge.summarize(p.id, { depth: "brief" }); setSum(r || { summaryText: "（无总结）", sourceBasis: "abstract" }); }
  ...
  return (
    <div className="dg-item">
      ...
      {sum && (
        <div className="dg-sum"><span className="dg-basis">● {sum.sourceBasis === "fulltext" || sum.sourceBasis === "full" ? "基于全文" : "基于摘要"}</span><div>{sum.summaryText || sum.text}</div></div>
      )}
      <div className="dg-acts">
        ...
        <button className="dg-act" onClick={doSum} disabled={summing}>{summing ? <><Loader size={13} className="rd-spin" /> 总结中…</> : <><Sparkles size={13} /> AI 总结</>}</button>
...
function SubDialog({ initial, onClose, onSave }) {
  ...
  const [autoSummarize, setAuto] = useState(initial ? initial.autoSummarize || "off" : "off");
    ...
    <div className="subs-modal" onMouseDown={(e) => { if (e.target === e.currentTarget) onClose(); }}>
      <div className="subs-dlg">
        ...
        <div className="subs-f"><label>成本闸（自动总结）</label>
          ...
          <span className="subs-hint">控制每次自动调用 LLM 的范围，防 token 失控。「关闭」只列新命中，不自动总结。</span>
        </div>
...
export default function Subscriptions({ pushToast, fetchedMeta = {}, fetchingMeta = {}, fetchTick = 0, onFetch: onFetchProp, onReadPaper }) {
  ...
  const subRunNow = useCallback(async (sub) => {
    ...
    if (r && r.ok && Array.isArray(r.hits) && r.hits.length) {
      ...
      pushToast && pushToast("已检索：新增 " + r.hits.length + " 条待读");
    } else {
      ...
      <div className="digest">
        ...
        <div className="dg-list">
          {loading ? (
            ...
            shown.map((s) => (s.enabled !== false && unread(s).length) ? (
              <div key={s.id}>
                <div className="dg-grp-h">{subKind(s) === "journal" ? <BookText size={15} style={{ color: "var(--gold)" }} /> : <Rss size={15} style={{ color: "var(--gold)" }} />}<h4>{subLabel(s)}</h4><span className="ct">待读 {unread(s).length}</span><span className="ln" /></div>
                {unread(s).map((p) => (
                  <DigestItem
                    key={p.id}
                    p={p}
