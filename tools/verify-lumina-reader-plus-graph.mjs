...
ok(has(rp, "PROMPTS") && /flowmap:\s*"[^"]*nodes/.test(rp) && has(rp, "只画正文写明的环节"), "flowmap prompt：产 nodes+edges JSON、只画正文写明环节、不补未述步骤、不把相关画成因果");
ok(has(rp, "async function runFlowmap"), "runFlowmap 实现");
ok(has(rp, "valid.has(p)"), "节点 pageRefs 经真实页码集过滤");
ok(has(rp, "ids.has(e.from) && ids.has(e.to)"), "引用不存在节点的边被丢弃");
ok(has(rp, "graph,") && has(rp, 'sourceBasis: "fulltext"'), "runFlowmap 返回 graph 信封（sourceBasis=fulltext）");
...
ok(has(rd, "env.graph && Array.isArray(env.graph.nodes)"), "EnvelopeCard 据 env.graph 路由到 GraphCard");
ok(rd && rd.indexOf("env.graph && Array.isArray") > rd.indexOf("if (!env) return null;"), "graph 路由在 null 守卫之后");
...
ok(/FlowmapTool[\s\S]{0,800}readerAnalyze\("flowmap"/.test(rd), "FlowmapTool 调 readerAnalyze('flowmap')");
ok(/FlowmapTool[\s\S]{0,800}saveCachedAnalysis/.test(rd), "FlowmapTool 结果缓存 saveCachedAnalysis");

console.log("\n[6] 图标 / CSS / 平衡");
ok(/import \{[^}]*\bWorkflow\b[^}]*\bMap\b[^}]*\} from "lucide-react"/.test(rd), "Workflow / Map 图标已导入");
ok(true, "Map 与 lucide 图标共存（已核对 Reader.jsx）");
