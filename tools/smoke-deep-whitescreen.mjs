#!/usr/bin/env node
/** Reproduce 深读白屏 — CDP console exceptions after switching to deep zone */
...
console.log("── deep whitescreen probe ──");
...
await evalJs(cdp, `
  ...
  if (!b) throw new Error("深读 tab not found");
  b.click();
  return true;
`);
await sleep(1500);
...
const after = await evalJs(cdp, `return {
  ...
  deep: !!document.querySelector(".rd-deep-layout"),
  tools: document.querySelectorAll(".rd-tool").length,
  rootLen: document.getElementById("root")?.innerHTML?.length || 0,
};`);
console.log("after deep:", after);
...
try {
  ...
  const fp = path.join(OUT, "deep-whitescreen.png");
